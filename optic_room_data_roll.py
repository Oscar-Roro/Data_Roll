...
base_dir = sys.path[0]
add_dir = "acqui-pics//2025_10_28_02" #! write "User/Documents" not "/User/Documents" 
...
def parse_fname(name):
    m = re.match(r"gn(\d+)_n(\d+)_t([\d\+\-\w]+)_gate_DDG_width([\d\+\-\w]+)_([\d\-]+)_([\d\-]+).npz",name) #(\d+\w\d+) or [\d\w]+
    if m:
        gain = int(m.group(1))
        npics = int(m.group(2))
    ...
    for gate_width_s, g in df.groupby(fixed): #all rows that have the same "column_value" are grouped, g = a smaller DataFrame containing only those rows
        ...
        if g["n_clusters"].dtypes == object: 
            ax.plot(g["texp_s"]*1e3,          # X-axis
                     ...
                     label=f"{gate_width_s*1e6:.2f}$\mu s$"
                     )
        else:
...
def plot_histograms(df,threshold, var="n_clusters_mean", group_var=None, bins=20, figsize=(8,5)):
    plt.figure(num=f" Histogram clusters thr:{threshold}",figsize=figsize)
    ...
    if group_var:
        ...
        plt.hist(df[var], bins=bins, color='skyblue', edgecolor='black', alpha=0.7)

    plt.xlabel(var)
    plt.ylabel("Frequency")
    plt.title(f"Histogram of {var}" + (f" grouped by {group_var}" if group_var else ""))
...
# 'n_frames','n_clusters_mean''n_clusters_std'}
    ...
    # plot_histograms(df, threshold, var="n_clusters_mean", group_var="gain")
